...
## Keys can contain any character but newline and double colon. Value can be anything but newline.
## Any text can be inserted in between lines. Matrices are stored as semicolon delimited values, 
## space characters are not important, however semicolon must be right after a numeric value. 
## Sections are used e.g. for multiple keys with same values.
##
## Example:
## @example
